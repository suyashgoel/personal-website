...
function RecommendationList({
  ...
  return (
    <div className="space-y-2">
      <p className="text-sm font-light text-foreground/50 tracking-tight">
        More like{' '}
        ...
        <ul className="flex flex-col gap-1">
          {recommendations.map(rec => (
            <li key={rec.slug}>
              ...
              >
                <span className="text-md font-light tracking-tight truncate">
                  {rec.title}
                </span>
...
export function RecommendationsSection({
  ...
  return (
    <section className="mt-8 pt-10 border-t">
      <div className="flex flex-col sm:flex-row justify-end items-start">
        <div className="flex flex-col gap-6 w-full sm:w-auto sm:min-w-72">
          {hasQueryRecs && (
            <RecommendationList
              title={query}
